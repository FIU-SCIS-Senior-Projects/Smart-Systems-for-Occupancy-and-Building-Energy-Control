...
$sql = "SELECT * FROM zone_occupancy WHERE";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
		$list[] = array('time_stamp' => $row["time_stamp"], 'occupancy' => $row["occupancy"], 'zone_description_region_id' => $row["zone_description_region_id"]);
    }
